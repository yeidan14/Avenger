<%-- 
    Document   : menu
    Created on : 11/11/2019, 10:47:53 PM
    Author     : Daniel
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<div class="sidebar">
      <div class="sidebar-header">
        <div>
          <a href="../index.html" class="sidebar-logo"><span>Avenger</span></a>
          <small class="sidebar-logo-headline">Yeison Daniel Manrique Camacho</small>
        </div>
      </div><!-- sidebar-header -->
      <div id="dpSidebarBody" class="sidebar-body">
        <ul class="nav nav-sidebar">
          <li class="nav-label"><label class="content-label">Paginas Necesarias</label></li>
          <li class="nav-item show">
            <a href="" class="nav-link with-sub active"><i data-feather="box"></i> Avenger</a>
            <nav class="nav nav-sub">
              <a href="" class="nav-sub-link active">Registrar Heroe</a>
              <a href="" class="nav-sub-link">Listar Heroes</a>
              <a href="dashboard-three.html" class="nav-sub-link">Registrar Peliculas</a>
            </nav>
          </li>
         


      </div><!-- sidebar-body -->
    </div>

      
